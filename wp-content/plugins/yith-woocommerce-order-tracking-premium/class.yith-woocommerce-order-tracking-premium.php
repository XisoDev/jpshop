	...
	class YITH_WooCommerce_Order_Tracking_Premium extends YITH_WooCommerce_Order_Tracking {
		...
		function show_order_tracking_metabox( $post ) {
			...
			<div class="track-information">
                ...
                <p>
                    <label
                            for="ywot_tracking_postcode"> <?php _e( 'Tracking postcode: (optional)', 'yith-woocommerce-order-tracking' ); ?></label>
                    <br />
                    <input style="width: 100%" type="text" name="ywot_tracking_postcode" id="ywot_tracking_postcode"
                           placeholder="<?php _e( 'Enter tracking postcode', 'yith-woocommerce-order-tracking' ); ?>"
		...
		function show_tracking_information( $order, $pattern, $prefix = '', $output = 'frontend' ) {
			...
			if ( 'email' == $output ) {
				...
				if ( $url ) {
				    $tracking_link = apply_filters( 'yith_ywcot_tracking_link','<br><a style="color:#3980c1; display:block;" href="' . $url . '" target="_blank">' . $text . "</a>", $url, $text );
		...
		public function get_picked_up_message( $order, $pattern = '', $isemail='' ) {
			...
			$message = apply_filters('yith_ywot_picked_up_message',str_replace(
				...
				$pattern ),$pattern,$carrier_name,date_i18n( get_option( 'date_format' ), strtotime( $data->get_pickup_date() ) ),$this->get_advanced_track_code( $data->get_tracking_code() ),$isemail);
