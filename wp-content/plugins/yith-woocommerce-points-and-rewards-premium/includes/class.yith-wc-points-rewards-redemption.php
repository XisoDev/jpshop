...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Redemption' ) ) {
	...
	class YITH_WC_Points_Rewards_Redemption {
				...
				if ( version_compare( WC()->version, '2.7', '<' ) ) {
					...
					if ( ! $valid ) {
						$args = array(
							...
							'free_shipping'  => YITH_WC_Points_Rewards()->get_option( 'allow_free_shipping_to_redeem', 'no' ) == 'yes',
		...
		public function update_discount() {
			...
			if ( $coupon = $this->check_coupon_is_ywpar( $applied_coupons ) ) {
				...
				if ( $max_discount ) {
					...
					$apply_coupon = ( current_filter() == 'woocommerce_cart_item_removed' );
		...
		public function get_conversion_rate_rewards( $currency = '' ) {
			...
			$conversion           = isset( $conversions[ $currency ] ) ? $conversions[ $currency ] : array( 'money' => 0, 'points' => 0 );
				...
				if ( YITH_WC_Points_Rewards()->get_option( 'rewards_points_for_role' ) == 'yes' ) {
					if ( ! empty( $current_user->roles ) ) {
						foreach ( $current_user->roles as $role ) {
							...
							$c = isset( $c[ $currency ] ) ? $c[ $currency ] : array( 'money' => 0, 'points' => 0 );
		...
		public function get_conversion_percentual_rate_rewards( $currency = '' ) {
			...
			$conversion  = isset( $conversions[ $currency ] ) ? $conversions[ $currency ] : array( 'points' => 0, 'discount' => 0 );
				...
				if ( YITH_WC_Points_Rewards()->get_option( 'rewards_points_for_role' ) == 'yes' ) {
					if ( ! empty( $current_user->roles ) ) {
						foreach ( $current_user->roles as $role ) {
							...
							$c = isset( $c[ $currency ] ) ? $c[ $currency ] : array( 'points' => 0, 'discount' => 0 );
			...
			if ( $this->get_conversion_method() == 'fixed' ) {
				...
				if ( $subtotal_cart != 0 ) {
					...
					if ( $points_usable >= $max_points ) {
						...
						$this->max_percentual_discount = round( $points_usable / $conversion['points'] ) * $conversion['discount'];
		...
		public function check_coupons_in_cart( $coupon_code ) {
			...
			$ywpar_coupon_in_cart = $this->check_coupon_is_ywpar( $coupons_in_cart );
			...
			switch ( $other_coupons ) {
				...
				case 'ywpar':
					...
					break;
				case 'wc_coupon':
					if ( ( $ywpar_added_coupon instanceof WC_Coupon ) && count( $applied_coupons ) > 1 ) {
						WC()->cart->remove_coupon( $coupon_code );
						$message = 'removed_par';
					} elseif ( $ywpar_coupon_in_cart instanceof WC_Coupon ) {
