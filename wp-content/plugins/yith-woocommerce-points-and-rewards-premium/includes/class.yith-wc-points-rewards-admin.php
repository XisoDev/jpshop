...
if ( !class_exists( 'YITH_WC_Points_Rewards_Admin' ) ) {
	...
	class YITH_WC_Points_Rewards_Admin {
        ...
        public function __construct() {
	        ...
	        /* Ajax action for apply points previous order */
	        add_action( 'wp_ajax_ywpar_fix_expiration_points', array( $this, 'fix_expiration_points' ) );
	        add_action( 'wp_ajax_nopriv_ywpar_fix_expiration_points', array( $this, 'fix_expiration_points' ) );
	    ...
	    public function new_expiration_mode_message(){
	        ...
	        if ( ! current_user_can( 'manage_options' )  || 'from_1.3.0' == get_option( 'yit_ywpar_expiration_mode' ) || isset( $_COOKIE['ywpar_notice'] ) || apply_filters('ywpar_expiration_old_mode', false ) ) {
	        	return;
			}
				//since 1.3.0!
	         ?>
				<div id="ywpar-notice-is-dismissable" class="error notice is-dismissible">
					...
					<p>
				        <?php _e( 'Due to some bugs, the point expiration system has been disabled.', 'yith-woocommerce-points-and-rewards' ); ?>
					</p>

					<p>
						<a href="<?php echo admin_url( "admin.php?page=".$this->_panel_page."&tab=expiration" ) ?>"><?php echo __( 'To enable it, please follow these instructions.', 'yith-woocommerce-points-and-rewards' ) ?></a>
					</p>
				</div>
        ...
        public function enqueue_styles_scripts() {
	        ...
	        if ( ( isset( $_GET['page'] ) && $_GET['page'] == 'yith_woocommerce_points_and_rewards' ) ) {
		        ...
		        wp_enqueue_style( 'select2', str_replace( array(
				        'http:',
				        'https:'
		        ...
		        wp_enqueue_script( 'yith_ywpar_admin', YITH_YWPAR_ASSETS_URL . '/js/ywpar-admin' . YITH_YWPAR_SUFFIX . '.js', array(
			        'jquery',
			        'jquery-ui-sortable'
		        ), YITH_YWPAR_VERSION, true );
		        ...
		        wp_localize_script( 'yith_ywpar_admin', 'yith_ywpar_admin', array(
			        ...
			        'import_points_import_file_empty' => __( 'The file import is empty', 'yith-woocommerce-points-and-rewards' ),
			        //from 1.2.0
        ...
        public function customers_tab() {
            ...
            if ( isset( $_REQUEST['action'] ) ) {
                ...
                if ( $_REQUEST['action'] == 'save' && wp_verify_nonce( $_POST['ywpar_update_points'], 'update_points' ) ) {
                    ...
                    YITH_WC_Points_Rewards()->register_log( $user_id, 'admin_action', '', $points_to_add );
...
<?php
	     ...
	     * @return  void
	     */
	    public function fix_expiration_points() {

		    check_ajax_referer( 'fix_expiration_points', 'security' );
