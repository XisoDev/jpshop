...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Frontend' ) ) {
	...
	class YITH_WC_Points_Rewards_Frontend {
		...
		public function ywpar_add_points_menu_items( $wc_menu ) {
			...
			$wc_menu[ $this->endpoint ] = YITH_WC_Points_Rewards()->get_option( 'my_account_page_label' );
		...
		public function print_messages_in_cart() {
			...
			if ( ! empty( $message ) ) {
				...
				printf( '<div id="yith-par-message-cart" class="%s">%s</div>', $classes, $message );
			}
		...
		private function get_cart_message( $total_points = 0 ) {
			...
			if ( $total_points >= 1 && isset( $discount ) ) {
				$message = str_replace( '{points_label}', $plural, $message );
				$message = str_replace( '{price_discount_fixed_conversion}', wc_price( $discount ), $message );
			} else {
				$message = str_replace( '{points_label}', $singular, $message );
			}
