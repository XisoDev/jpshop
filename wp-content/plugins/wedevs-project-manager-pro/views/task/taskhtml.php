            ...
            <div class="cpm-col-7">
                ...
                if ( $task->completed == '1' && $task->completed_by ) {
                    ...
                    <span class="cpm-completed-by">
                        <?php printf( __( 'Completed by %s on %s', 'cpm-pro' ), cpm_url_user( $task->completed_by, true ), $completion_time ) ?>
                    </span>

                    <span class="cpm-assigned-users hidden">
                        <?php
                    ...
                    if ( $start_date != '' || $task->due_date != '' ) {
                        $task_status_wrap = ( !empty( $task->due_date ) && ( date( 'Y-m-d', time() ) > date( 'Y-m-d', strtotime( $task->due_date) ) )) ? 'cpm-due-date' : 'cpm-current-date';
                    ?>

                    <span class="hidden <?php echo $task_status_wrap; ?>">
                        <?php
