...
class CPM_Pro_Files {
    ...
    function register_post_type() {
        register_post_type( 'cpm_docs', array(
            'label'               => __( 'Documents', 'cpm-pro' ),
            'description'         => __( 'Documents', 'cpm-pro' ),
            'public'              => false,
            ...
            'show_in_json'        => true,
            'labels'              => array(
                'name'               => __( 'Documents', 'cpm-pro' ),
                'singular_name'      => __( 'Document', 'cpm-pro' ),
                'menu_name'          => __( 'Documents', 'cpm-pro' ),
                'add_new'            => __( 'Add Document', 'cpm-pro' ),
                'add_new_item'       => __( 'Add New Document', 'cpm-pro' ),
                'edit'               => __( 'Edit', 'cpm-pro' ),
                'edit_item'          => __( 'Edit Document', 'cpm-pro' ),
                'new_item'           => __( 'New Document', 'cpm-pro' ),
                'view'               => __( 'View Document', 'cpm-pro' ),
                'view_item'          => __( 'View Document', 'cpm-pro' ),
                'search_items'       => __( 'Search Documents', 'cpm-pro' ),
                'not_found'          => __( 'No Documents Found', 'cpm-pro' ),
                'not_found_in_trash' => __( 'No Documentst Found in Trash', 'cpm-pro' ),
                'parent'             => __( 'Parent Document', 'cpm-pro' ),
            ),
        ) );
    }
    ...
    function show_message() {
        if ( isset( $_GET['pid'] ) OR isset( $_GET['project_id'] ) ) {
            ...
            wp_localize_script( 'cpm_pro_files', 'CPM_pro_files', array(
                'nonce'          => wp_create_nonce( 'cpm_nonce' ),
                'current_project' => $pid,
                'project_obj' => $project_obj,
                'base_url'        => CPM_PRO_URL,
                'static_text'     => array(
                    'create_folder'         => __( 'Create a folder', 'cpm-pro' ),
                    'upload_file'           => __( 'Upload a file', 'cpm-pro' ),
                    'create_doc'            => __( 'Create a doc', 'cpm-pro' ),
                    'create_document'       => __( 'Create a document', 'cpm-pro' ),
                    'link_google_doc'       => __( 'Link to Docs', 'cpm-pro' ),
                    'back_previus'          => __( 'Back to previous', 'cpm-pro' ),
                    'file_upload'           => __( 'File uploads', 'cpm-pro' ),
                    'attach_file'           => __( 'Attach a File', 'cpm-pro' ),
                    'make_file_private'     => __( 'Make files private.', 'cpm-pro' ),
                    'make_private'          => __( 'Make this private.', 'cpm-pro' ),
                    'submit'                => __( 'Submit', 'cpm-pro' ),
                    'file_folder_not_found' => __( 'Folder or Documents not found. Please create new folder or document.', 'cpm-pro' ),
                    'folder_create'         => __( 'Create Folder', 'cpm-pro' ),
                    'title'                 => __( 'Title', 'cpm-pro' ),
                    'delete_file'           => __( 'Delete File', 'cpm-pro' ),
                    'to_attach'             => __( 'To attach', 'cpm-pro' ),
                    'select_file'           => __( 'select files', 'cpm-pro' ),
                    'from_computer'         => __( 'from your computer', 'cpm-pro' ),
                    'google_link'           => __( 'Link for  Docs', 'cpm-pro' ),
                    'download'              => __( 'Download', 'cpm-pro'),
                    'note'                  => __( 'Note', 'cpm-pro' ),
                    'view_current_post'     => __( 'View Current Post', 'cpm-pro' ),
                    'view_on_google'        => __( 'View Online', 'cpm-pro' ),
                    'attachment'            => __( 'Attachments', 'cpm-pro' ),
                    'comments'              => __( 'Comments', 'cpm-pro' ),
                    'on'                    => __( 'On', 'cpm-pro' ),
                    'by'                    => __( 'by', 'cpm-pro' ),
                    'load_more_file'                    => __( ' Load More Files', 'cpm-pro' ),
                    'add_comment'           => __( 'Add Comment', 'cpm-pro' ),
                    'update_doc'            => __( 'Update Doc', 'cpm-pro' ),
                    'cancel_edit'          => __( 'Cancel Edit', 'cpm-pro' ),
                    'revisions'             => __( 'Revisions', 'cpm-pro' ),
                    'no_revision'           => __( 'No Revision', 'cpm-pro' ),
                    'delete_file_confirm'   => __( 'Are you sure to delete this file?', 'cpm-pro' ),
                    'delete_folder'         => __( 'Are you sure to delete this folder?', 'cpm-pro' ),
                    'change_file_privacy'   => __( 'Are you sure to change privacy for this file or folder?', 'cpm-pro' ),
                    'empty_comment'         => __( 'Please write something in comments!', 'cpm-pro' ),
                    'document_n_file'       => __('Documents & Files', 'cpm-pro'),
                    'file_page_intro' => __("Access all the files from all threads in one place. Store your project related files, docs, images or any other files on-site. Makes sharing and storing easy and convenient, plus you can add privacy here too.", "cpm-pro"),
                    'benefits_of_doc' => __("Benefits of Documents", "cpm-pro"),
                    'integrate_easily' => __("Integrate easily with projects or tasks.", "cpm-pro"),
                    'shared_storage' => __("Shared storage keeps all data secure.", "cpm-pro"),
                    'file_share_privacy' => __("File sharing privacy.", "cpm-pro"),
                ),
            ) );
        }
    }
    ...
    function files_scripts() {
        ...
        if ( isset( $_GET['tab'] ) AND $_GET['tab'] == 'files' ) {
            $dependant_scripts = array(
                'jquery',
                'cpm_admin',
                'cpm-vue',
            );

            foreach ($dependant_scripts as $script) {
                wp_enqueue_script( $script );
            }

            wp_enqueue_script( 'cpm-vuejs', CPM_PRO_URL . '/assets/js/vue.min.js', false, time(), true );
    ...
    function file_new_uplaod() {
        ...
        if ( empty( $response['success'] ) ) {
            $response['error'] = __( 'Pleae choose any file first', 'cpm-pro' );
        }
    ...
    function new_folder_create() {
        ...
        if ( cpm_user_can_access( $project_id, 'upload_file_doc' ) ) {
            ...
            $response['error'] = __( 'You have not access to creat folder.', 'cpm-pro' );
        }
        echo json_encode( $response );
        exit();
    }
    ...
    function delete_uploded_file() {
        ...
        if ( $file_relation->type == 'doc' OR $file_relation->type == 'google_doc'  ) {
            ...
            if ( get_current_user_id() == $file_relation->created_by ) {
                ...
                $response['error'] = __( 'Permission Problem', 'cpm-pro' );
            }
        }
    ...
    function cpm_pro_create_comment() {
        ...
        if ( $comment_id ) {
            ...
            //  $comment['avata'] = ($comment->comment_author_email) ;
    ...
    function cpm_pro_doc_revision() {
        ...
        if ( $revision_list ) {
            foreach ( $revision_list as $rev ) {
                ...
                $sname           = (strlen( $rev->post_title ) > $this->_files_name_show) ? substr( $rev->post_title, 0, $this->_files_name_show ) . "..." : $rev->post_title;
                $file_obj        = array(
                    'id'            => $rev->id,
